...
As an alternative to this EM approach we can find the MLE by classical optimization techniques. We notice that for $x_i \geq y_i \Leftrightarrow u_i = 1$ we get

$$ p(Z_i \leq z, U_i = 1) = \int_0^{z} \int_0^{x_i} \lambda_0 e^{-\lambda_0 x_i} \lambda_1 e^{-\lambda_1 x_i} dy_i dx_i \\
...
We do the equivalent for $y_i \geq x_i \Leftrightarrow u_i = 0$ and get

$$p(Z_i = z \lvert U_i = 0) = \lambda_1 e^{-\lambda_1 z_i}(1-e^{-\lambda_0 z_i})$$

We can then formulate the likelihood,
$$
L(\lambda_0, \lambda_1 \lvert \mathbf{u}, \mathbf{z}) = \prod_{u_i = 1} \lambda_0 e^{-\lambda_0 z_i}(1-e^{-\lambda_1 z_i}) \prod_{u_i = 0} \lambda_1 e^{-\lambda_1 z_i}(1-e^{-\lambda_0 z_i}),
$$

and the log-likelihoodof the observations $(\mathbf{z}, \mathbf{u})$
$$
\begin{align*}
\ell(\lambda_0, \lambda_1 \lvert \mathbf{u}, \mathbf{z}) &= \sum_{u_i = 1} \log \lambda_0 -\lambda_0 z_i + \log (1-e^{-\lambda_1 z_i}) \\
&+\sum_{u_i = 0} \log \lambda_1 -\lambda_1 z_i + \log(1-e^{-\lambda_0 z_i})
\end{align*}
$$
