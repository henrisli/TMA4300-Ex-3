...
From \ref{fig:conv_lamb} we see that the algorithm converges to the values `r lambda_orig` after only 2-4 iterations. 

## 3).

We now want to investigate the bias and standard deviations of this EM estimator of the MLE of $(\lambda_0, \lambda_1)$. To do this we implement a bootstrap algorithm. The algorithm is summarized in the following pseudocode. 

```{r, eval = FALSE}
for b in (1, 2, ... bootstrap_samples) do

  u bootstrap = sample with replacement(u)
  z bootstrap = sample with replacement(z)
   
  lambda0, lambda1 = run EM algorithm with u bootstrap, z bootstrap
  
  store lambda0, lambda1 as a single bootstrap estimate of the MLE of lambda_0, lambda_1
```
...
From the lines in figure \ref{fig:bs_lamb} we see that the mean $(\bar{\lambda}_0^*, \bar{\lambda}_1^*)$ of the bootstrap samples correspond well with the original EM estimate. We observe the values

```{r, echo=T, eval=T}
cat("BS mean", apply(lambda_bs,2,mean), "\n")
cat("origian EM", lambda_orig)
```

which correspond to the biases

```{r, echo=T, eval=T}
cat("Bias of the bootstrapped samples: \n")
cat(apply(lambda_bs,2,mean) - lambda_orig, "\n")
```

we see that the bootstrap estimate of the bias in our EM estimator non-negligible. We observe that the correlation between the bootstrap estimates of $lambda_0$ and $lambda_1$, 

```{r, echo=T, eval=T}
cat("Corr(lambda_0, lambda_1):", cor(lambda_bs)[1, 2], " ")
```

is weakly negative.

From the width of the distribution of the bootstrap samples we see that there is significant variance in the bootstrap estimates, especially in $\lambda_1$. 

```{r, echo=T, eval=T}
cat("lambda_0, var:", cov(lambda_bs)[1, 1], " ")
cat("stdev:", sqrt(cov(lambda_bs)[1, 1]), "\n")
cat("lambda_1, var:", cov(lambda_bs)[2, 2], " ")
cat("stdev:", sqrt(cov(lambda_bs)[2, 2]), "\n")
```

In fact the standard deviations are about a magnitude larger than the bias. As the variance in this case is the main contributor to the error of the estimator we prefer the original EM estimate instead of the bias-corrected estimate. The bias-corrected estimator will give larger variance and thus less accurate estimates of the intensities $\lambda_0$ and $\lambda_1$. 

## 4).



To get the loglikelihood of the full sample, we first take the log of $f_{Z_i}(z_i)$ to get
$$\log(f_{Z_i}(z_i)) = \begin{cases}\log(\lambda_1)-z_i(\lambda_0+\lambda_1)+\log(e^{\lambda_0z_i}-1), &\text{ if }u_i=0,\\\log(\lambda_0)-z_i(\lambda_0+\lambda_1)+\log(e^{\lambda_1z_i}-1), &\text{ if }u_i=1. \end{cases}$$
We can then find the loglikelihood of the total sample as,
\begin{equation}
\begin{split}
\log\big(f_{\mathbf{Z},\mathbf{U}}(\mathbf{z},\mathbf{u})\big) &= \log\Big(\prod_{i=1}^n f_{Z_i}(z_i)\Big) = \sum_{i=1}^n \log(f_{Z_i}(z_i))\\
...
\label{eq:ll}
\end{equation}
To find analytical formulas for the maximum likelihood estimators of $\hat{\lambda}_0$ and $\hat{\lambda}_1$, we differentiate the loglikelihood w.r.t. $\lambda_0$ and $\lambda_1$,

$$ \frac{\partial \log\big(f_{\mathbf{Z},\mathbf{U}}(\mathbf{z},\mathbf{u})\big)}{\partial \lambda_0} = \frac{N_1}{\lambda_0} - \sum_{i=1}^n z_i + \underset{u_i=0}{\sum_{i=1}^n}\frac{z_ie^{z_i\lambda_0}}{e^{z_i\lambda_0}-1},$$
$$ \frac{\partial \log\big(f_{\mathbf{Z},\mathbf{U}}(\mathbf{z},\mathbf{u})\big)}{\partial \lambda_1} = \frac{N_0}{\lambda_1} - \sum_{i=1}^n z_i + \underset{u_i=1}{\sum_{i=1}^n}\frac{z_ie^{z_i\lambda_1}}{e^{z_i\lambda_1}-1},$$
with $N_0=\sum_{i=1}^nI(u_i=0)$ and $N_1=\sum_{i=1}^nI(u_i=1)$.


As an alternative to this EM approach we can find the MLE by classical optimization techniques. We first calculate the probablity that $Z_i \leq z$ and $u_i = 1$. We can find this by integrating the joint pdf of $(x_i, y_i)$
...
We see from figure \ref{fig:llik_lamb} that the optimization scheme correctly finds the MLE $(\hat{\lambda}_0, \hat{\lambda}_1)$ of $(\lambda_0, \lambda_1)$ and that it corresponds to the original EM-estimate of $(\\lambda_0, \\lambda_1)$.  and the bootstrap mean of $(\\lambda_0, \\lambda_1)$
