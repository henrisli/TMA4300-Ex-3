...
LEGGE TIL FACET WRAP HER SLIK AT VI FÅR CAPTION??!?!?!?!
...
We let $\{x_i\}_{i=1,\dots,n}$ be iid $\text{Exp}(\lambda_0)$ and $\{y_i\}_{i=1,\dots,n}$ be iid $\text{Exp}(\lambda_1)$. We assume that we observe neither $x_i$ nor $y_i$ directly, but rather observe
...
Based on the observed $(z_i,u_i) \quad i= 1,\dots,n$ we want use the Expecation-Maximization algorithm to find the maximum likelihood estimates for $(\lambda_0,\lambda_1)$. We first formulate the conditional probability of $\mathbf{x} = (x_1,\dots,x_n)$ and $\mathbf{y} = (y_1,\dots,y_n)$ given $(\lambda_0, \lambda_1)$. 
...
Taking the partial derivative w.r.t. to $\lambda_0$ we get
