...
# We now want df to be the Fisher-distribution
rm(df)
...
dfem = data.frame(n = 1:dim(lambda)[1], lambda)

ggplot() + geom_line(data = dfem, aes(x = n, y = X1, col="lambda_0")) + geom_line(data = dfem, aes(x = n, y = X2, col="lambda_1")) + ylab("")
```
...
dfbs1 = data.frame(idx = 1, x = lambda_bs[, 1], x0 = lambda_orig[1], mean = mean(lambda_bs[, 1]))
dfbs2 = data.frame(idx = 2, x = lambda_bs[, 2], x0 = lambda_orig[2], mean = mean(lambda_bs[, 2]))
...
If we condition on the value of $z_i$ we get that $u_i$ is bernoulli-distributed with probabilites $p(u_i = x_i \lvert z_i) = \frac{f_{X_i}(z_i)}{f_{X_i}(z_i) + f_{Y_i}(z_i)}$ and $p(u_i = y_i \lvert z_i) = \frac{f_{Y_i}(z_i)}{f_{X_i}(z_i) + f_{Y_i}(z_i)}$
