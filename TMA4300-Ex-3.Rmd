...
$$ f_{X_i, Y_i} (x_i, y_i \lvert \lambda_0, \lambda_1) = \lambda_0 e^{-\lambda_0 x_i } \lambda_1 e^{-\lambda_1 y_i }$$
...
\begin{align}\log f_{\mathbf{Z}, \mathbf{U}}(\mathbf{z}, \mathbf{u}) 
...
\begin{align} \frac{\partial f_{\mathbf{Z}, \mathbf{U}}(\mathbf{z}, \mathbf{u})}{\partial \lambda_0} 
...
\begin{align} \frac{\partial f_{\mathbf{Z}, \mathbf{U}}(\mathbf{z}, \mathbf{u})}{\partial \lambda_1} 
...
lambda_optim = optim(par=c(4, 12), fn=loglik, control = list(fnscale = -1, maxit=1000))$par

x0 = seq(1, 10, length.out = 30)
x1 = seq(1, 20, length.out = 30)
...
ggplot() + geom_contour(data = df, aes(x = lambda_0, y = lambda_1, z = llik), binwidth=10)  + geom_point(data = df_dot1, aes(x = lambda_0, y = lambda_1, col="EM-estimate"), size=3) + geom_point(data = df_dot2, aes(x = lambda_0, y = lambda_1, col="optimized"), size=3) + geom_point(data = df_dot3, aes(x = lambda_0, y = lambda_1, col="bootstrap mean"), size=3)
```
