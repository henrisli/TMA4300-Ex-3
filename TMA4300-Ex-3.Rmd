...
\begin{align*}
P(z_i\leq z) = F_{Z_i}(z_i)&=\begin{cases}\int_0^{z_i} \int_0^y \lambda_0e^{-\lambda_0x_i}\lambda_1e^{-\lambda_1y_i}dxdy, &\text{ if }u_i=0,\\\int_0^{z_i} \int_0^x \lambda_0e^{-\lambda_0x_i}\lambda_1e^{-\lambda_1y_i}dydx, &\text{ if }u_i=1,\end{cases}\\
&= \begin{cases}\frac{\lambda_1(\exp\{-z_i(\lambda_0+\lambda_1)\}-1)}{\lambda_0+\lambda_1}-e^{-\lambda_1z_i}+1, &\text{ if }u_i=0,\\\frac{\lambda_0(\exp\{-z_i(\lambda_0+\lambda_1)\}-1)}{\lambda_0+\lambda_1}-e^{-\lambda_0z_i}+1, &\text{ if }u_i=1. \end{cases}
\end{align*}

We then differentiate on $z_i$ to get the pdf of $z_i$ as,

$$f_{Z_i}(z_i) = \frac{\partial F_{Z_i}(z_i)}{\partial z_i} =  \begin{cases}\lambda_1(e^{-\lambda_1z_i}-e^{-z_i(\lambda_0+\lambda_1)}), &\text{ if }u_i=0,\\\lambda_0(e^{-\lambda_0z_i}-e^{-z_i(\lambda_0+\lambda_1)}), &\text{ if }u_i=1. \end{cases}$$
...
\end{align*}
--->
Classically, we would want to find the maximum-likelhood parameters $(\lambda_0, \lambda_1)$ by solving
$$ \frac{\partial \log f_{\mathbf{Z}, \mathbf{U}}(\mathbf{z}, \mathbf{u})}{\partial \lambda_0}=0, \quad \text{and} \quad \frac{\partial \log f_{\mathbf{Z}, \mathbf{U}}(\mathbf{z}, \mathbf{u})}{\partial \lambda_1} = 0.$$
...
loglik = function(x){
  ...
  # 2nd sum
  res = res - sum(log(1 + lambda_1 / lambda_0 * exp(-(lambda_1 - lambda_0)*z[which(u==1)])))
  # 3rd sum
  res = res - sum(log(1 + lambda_0 / lambda_1 * exp(-(lambda_0 - lambda_1)*z[which(u==0)])))
  return(res)
}
...
lambda_optim = optim(par=c(4, 12), fn=loglik2, control = list(fnscale = -1, maxit=1000, reltol=1e-16, ndeps=1e-6))$par
