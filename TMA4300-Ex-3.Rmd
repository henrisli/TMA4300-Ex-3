...
```
From plot \ref{fig:conv_lamb2} we see that the convergence in norm of the update, $\lVert (\lambda_0^{(t)}, \lambda_1^{(t)}) - (\lambda_0^{(t-1)}, \lambda_1^{(t-1)}) \rVert$, is slightly faster than linear.
